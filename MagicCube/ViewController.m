...
//    [eaglView setContentScaleFactor:[[UIScreen mainScreen] scale]];
    self.view = eaglView;
    ...
    _startTime = time(0);
    _consumeTime = 0;
    NSString *timeText = [NSString stringWithFormat:@"%02ld:%02ld:%02ld",(_consumeTime/60)/60,(_consumeTime/60)%60, _consumeTime%60];
...
- (void)drawFrame 
{
    ...
    [_timeLabel setString: [NSString stringWithFormat:@"%02ld:%02ld:%02ld",(temp/60)/60,(temp/60)%60, temp%60]];
...
- (CGPoint)locationWithUITouch:(UITouch *)touch View:(UIView *)view {
    CGPoint point = [touch locationInView:view];
    point.x = point.x * [view contentScaleFactor];
    point.y = point.y * [view contentScaleFactor];
    return point;
}
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (allTouches.count == 1) { //旋转整个魔方 或者 某个列
        _lastTouchPosition = [self locationWithUITouch:[allTouches anyObject] View:self.view];
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (allTouches.count == 1) {
        ...
        CGPoint currentPostion = [self locationWithUITouch:[allTouches anyObject] View:self.view];
